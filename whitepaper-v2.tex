...
\junk{
Adversarial uncertainty: In the adversarial model, we may assume that
there is no a priori information on the target locations and that they
are revealed one by one in an adversarial sequence. If we require a
tree that covers all targets as they materialize and connects them to
the root, we are looking at the well-studied {\em online Steiner tree
problem}. The natural greedy algorithm for this problem with $k$
targets has a competitive ratio of $O(\log k)$.
}
\smallskip
...
\smallskip
\BfPara{Information dissemination in highly dynamic networks} We will
conduct a comprehensive study of fully-distributed gossip algorithms
for the $k$-broadcast problem in dynamic networks.  In recent
work~\cite{dutta+prs:dynamic}, we studied the class of {\em
...
other than copying, storing, and forwarding them.  We show that {\em
  any}\/ forwarding algorithm will take $\Omega(nk/\log(n))$ steps,
thus resolving an open problem of~\cite{kuhn+lo:dynamic}.  Given that
almost any local greedy forwarding procedure completes $k$-broadcast
in $O(nk)$ steps in any dynamic network, our lower bound essentially
captures the severe limitations imposed by highly adversarial network
dynamics.  An attractive alternative to purely forwarding algorithms
is to use coding; recent work~\cite{haeupler+k:dynamic} has shown that
...
large and impractical our results establish a fundamental gap between
flow-based and coding-based dissemination procedures.
